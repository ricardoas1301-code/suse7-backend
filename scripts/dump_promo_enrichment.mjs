...
const NAME_FILTER = process.argv[3] ?? "Aumente";
...
for (const row of rows) {
  ...
  if (!name.toLowerCase().includes(NAME_FILTER.toLowerCase())) continue;
  ...
  const ui = resolvePromotionUiFinancials(row, { listingId: LISTING });
  console.log("ROW", JSON.stringify(row, null, 2));
  console.log("ITEMS", JSON.stringify(items[0] ?? null, null, 2));
  console.log("UI", {
    original: ui.original_price_brl,
    final: ui.final_price_brl,
    disc: ui.discount_amount_brl,
    pct: ui.discount_percent_display,
    rule: ui.panel_parity?.selected_rule,
    source: ui.final_price_source,
  });
}
