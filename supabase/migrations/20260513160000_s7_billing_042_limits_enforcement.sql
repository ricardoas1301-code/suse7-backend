-- BILLING 04.2 — limites operacionais, nomes de plano e base de uso mensal
...
UPDATE public.plans
SET
  display_name = COALESCE(display_name, NULLIF(trim(name), '')),
  marketing_name = COALESCE(marketing_name, NULLIF(trim(name), '')),
  slug = COALESCE(slug, NULLIF(trim(plan_key), ''))
