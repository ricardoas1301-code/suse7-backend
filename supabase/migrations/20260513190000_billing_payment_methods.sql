-- Canonical: billing_payment_methods (merged 20260513 + 20260518 orphans)
-- Posição: após billing compat (20260513180000), antes renewal_cycles (20260519120000)
...
CREATE TABLE IF NOT EXISTS public.billing_payment_methods (
  ...
  CONSTRAINT billing_payment_methods_card_type_check CHECK (card_type IN ('CREDIT', 'DEBIT'))
);
...
CREATE POLICY billing_payment_methods_insert_own
  ON public.billing_payment_methods FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY billing_payment_methods_update_own
  ON public.billing_payment_methods FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY billing_payment_methods_delete_own
  ON public.billing_payment_methods FOR DELETE USING (auth.uid() = user_id);
