// =============================================================================
// Acionamento manual — Relatório de Vendas → Notification Actions Engine
// Espelha o fluxo do Raio-X (MANUAL_SALE_RAYX) para MANUAL_SALES_REPORT.
// WhatsApp: imagem + Excel via processWhatsAppOutboxDispatch.
// E-mail: S7 Mail v1 + mesmos artefatos (imagem inline + Excel anexo).
// =============================================================================
...
import { patchSalesReportEmailOutboxShare } from "./salesReportOutboxShare.js";

const MANUAL_TYPE = "MANUAL_SALES_REPORT";
...
function normalizeTemplatePayload(raw) {
  ...
  return {
    periodo: String(src.periodo ?? "—").trim() || "—",
    conta: String(src.conta ?? "—").trim() || "—",
    vendas: String(src.vendas ?? "—").trim() || "—",
    faturamento: String(src.faturamento ?? "—").trim() || "—",
    lucro: String(src.lucro ?? "—").trim() || "—",
    margem: String(src.margem ?? "—").trim() || "—",
  };
}
 ...
 * }} input
 */
export async function triggerManualSalesReportNotification(supabase, input) {
  ...
  const correlationId = `sales-report.manual.${reportKey}.${channel}.${bucket}`;
  const recipientKey =
    channel === S7_NOTIFICATION_CHANNEL.WHATSAPP
      ...
      : String(input.recipientEmail ?? input.recipientId ?? "unknown")
          .trim()
          .toLowerCase() || "unknown";
  const idempotencyKey = `manual.sales-report:${reportKey}:${channel}:${recipientKey}:${bucket}`;

  logNotificationActions("MANUAL_SALES_REPORT_START", {
    seller_id: sellerId,
  ...
  const pub = await publishNotificationEvent(supabase, {
    ...
    entity_type: "sales_report_manual",
    entity_id: reportKey,
    source_module: "vendas_relatorio_modal",
  ...
  const deliveryFormat =
    input.deliveryFormat != null ? String(input.deliveryFormat).trim() : shareImageBase64 ? "image" : "text";
  ...
  if (channel === S7_NOTIFICATION_CHANNEL.EMAIL && dispatchId) {
    ...
    let emailGreetingName =
      input.recipientName != null && String(input.recipientName).trim() !== ""
        ? String(input.recipientName).trim()
        : null;

    if (!emailGreetingName) {
      const { data: profile } = await supabase
        .from("profiles")
        .select("display_name, nome_loja")
        .eq("id", sellerId)
        .maybeSingle();
      const candidate = profile?.display_name ?? profile?.nome_loja ?? null;
      emailGreetingName =
        candidate != null && String(candidate).trim() !== "" ? String(candidate).trim() : null;
    }

    const rendered = renderSalesReportManualEmailBody({
      recipientName: emailGreetingName,
    });

    await patchSalesReportEmailOutboxShare(supabase, dispatchId, {
  ...
  logNotificationActions("MANUAL_SALES_REPORT_COMPLETE", {
    seller_id: sellerId,
    report_key: reportKey,
    channel,
    ...
    live_audit:
      channel === S7_NOTIFICATION_CHANNEL.WHATSAPP
        ? auditManualSaleRayxWhatsAppLive({
            ...
            liveDestinationSource,
            smokeOverrideApplied,
            destinationTrace: destinationTrace ?? undefined,
          })
        : null,
    ...(destinationTrace ?? {}),
    duration_ms: Date.now() - routeStarted,
 ...
 * }} input
 */
export async function triggerManualSalesReportNotificationsBatch(supabase, input) {
  const channel = String(input.channel ?? "").trim().toLowerCase();
  const rawTargets = Array.isArray(input.recipientTargets) ? input.recipientTargets : [];
  if (rawTargets.length === 0) {
    return triggerManualSalesReportNotification(supabase, input);
  }
  ...
  for (const target of targets) {
    if (channel === S7_NOTIFICATION_CHANNEL.WHATSAPP) {
      ...
      const one = await triggerManualSalesReportNotification(supabase, {
        ...
        shareImageFilename: input.shareImageFilename ?? null,
    ...
    results.push(one);
    if (target.recipientId) selectedRecipientIds.push(String(target.recipientId));
    if (target.recipientEmail) selectedRecipientEmails.push(String(target.recipientEmail));
  }
  ...
  if (results.length === 0) {
    return {
      ok: false,
      success: false,
      error: channel === S7_NOTIFICATION_CHANNEL.EMAIL ? "INVALID_EMAIL" : "INVALID_PHONE",
      multi: true,
    };
  }
  ...
  return {
    ...
    selected_recipient_emails: selectedRecipientEmails,
    dispatches_created: dispatchesCreated,
    results,
