...
export default async function handleMlSalesSummary(req, res) {
  ...
  try {
    ...
    const { data: items, error: itemsErr } = await supabase
      ...
      .select("quantity, gross_amount, net_amount, external_listing_id")
      .eq("user_id", userId)
      .eq("marketplace", marketplace);
    ...
    /** Soma só em linhas com external_listing_id (o que pode ir para métricas por anúncio). */
    let gross_revenue_listing_lines = 0;
    let net_revenue_listing_lines = 0;
    let order_lines_without_listing_id = 0;
    ...
    for (const it of items || []) {
      ...
      const netPart = n != null && Number.isFinite(n) ? n : gross;
      net_revenue_total += netPart;

      const hasListing =
        it.external_listing_id != null && String(it.external_listing_id).trim() !== "";
      if (hasListing) {
        gross_revenue_listing_lines += gross;
        net_revenue_listing_lines += netPart;
      } else {
        order_lines_without_listing_id += 1;
      }
    }

    const { count: listing_metrics_row_count, error: metCountErr } = await supabase
    ...
    if (metCountErr) {
      console.error("[ml/sales-summary] metrics_row_count", metCountErr);
    }
    ...
    return res.status(200).json({
      ...
      gross_revenue_total,
      net_revenue_total,
      /** Gráfico “por anúncio” usa listing_sales_metrics; só linhas com anúncio entram aqui. */
      gross_revenue_listing_lines,
      net_revenue_listing_lines,
      order_lines_without_listing_id,
      listing_metrics_row_count: listing_metrics_row_count ?? null,
