/**
 * Persistência da ordem/capa local das imagens por anúncio (S1.21+ / S1.22).
 ...
 * @param {unknown} error
 */
export function isMissingPrimaryPictureSettingsTableError(error) {
  ...
  return (
    code === "42P01" ||
    code === "PGRST205" ||
    message.includes("marketplace_listing_primary_picture_settings") ||
...
function extrairSettingsDeRawJson(rawJson) {
  ...
  const nested = /** @type {Record<string, unknown>} */ (rawJson).s7_listing_primary_picture_settings;
 ...
 */
export async function carregarListingPrimaryPictureSettings(supabase, userId, listingId, listingRow = null) {
  try {
    const { data, error } = await supabase
      .from("marketplace_listing_primary_picture_settings")
      .select("primary_picture_id, primary_picture_url, ordered_picture_keys, updated_at, updated_by")
    ...
    if (error && !isMissingPrimaryPictureSettingsTableError(error)) {
      console.warn("[LISTING_PRIMARY_PICTURE_LOAD]", {
 ...
 * }} input
 */
export async function salvarListingPrimaryPictureSettings(supabase, userId, listingId, input) {
  const nowIso = new Date().toISOString();
  const payload = {
    listing_id: listingId,
    user_id: userId,
    primary_picture_id: input.primaryPictureId,
    primary_picture_url: input.primaryPictureUrl,
    ordered_picture_keys: input.orderedPictureKeys,
    updated_at: nowIso,
    updated_by: userId,
  };
  ...
  if (!isMissingPrimaryPictureSettingsTableError(upsertErr)) {
    console.error("[LISTING_PRIMARY_PICTURE_SAVE]", {
      listing_id: listingId,
  ...
  const rawNext = {
    ...rawCurrent,
    s7_listing_primary_picture_settings: {
      primary_picture_id: payload.primary_picture_id,
      primary_picture_url: payload.primary_picture_url,
      ordered_picture_keys: payload.ordered_picture_keys,
      updated_at: nowIso,
      updated_by: userId,
    },
  };
