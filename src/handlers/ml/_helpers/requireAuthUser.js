...
export async function requireAuthUser(req) {
  if (!config.supabaseUrl?.trim() || !config.supabaseServiceRoleKey?.trim()) {
    return { error: { status: 503, message: "Configuração do banco indisponível", code: "CONFIG_ERROR" } };
  }
  ...
  try {
    ...
    return {
      error: {
        status: 503,
        message: "Configuração do banco indisponível",
        code: "CONFIG_ERROR",
        details: error?.message ?? "auth bootstrap failed",
      },
    };
  }
}
