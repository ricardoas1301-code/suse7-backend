// ======================================================================
// POST|GET /api/jobs/billing-process-renewals
// Proteção: X-Job-Secret quando JOB_SECRET estiver configurado.
// ======================================================================
...
export async function handleJobsBillingProcessRenewals(req, res) {
  ...
  try {
    ...
    const result = await processBillingRenewalEngine(supabase, {
      providerApi,
      requestId: traceId,
      jobName: "billing-process-renewals",
      limit,
    });
    return ok(res, { ok: true, job: "billing-process-renewals", ...result, traceId });
  } catch (error) {
    return fail(
      res,
      {
        code: "RENEWAL_JOB_FAILED",
        message: error instanceof Error ? error.message : "Falha ao processar renovações.",
      },
      500,
      traceId
    );
