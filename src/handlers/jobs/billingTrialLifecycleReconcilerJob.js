// ======================================================================
// POST|GET /api/jobs/billing-trial-lifecycle-reconciler
// S1.HF.6.9A.11 — reconciliador do ciclo trial (alertas + expiração)
...
import { runBillingTrialLifecycleReconcilerJob } from "../../billing/jobs/billingTrialLifecycleReconcilerJob.js";
 ...
 */
export default async function billingTrialLifecycleReconcilerJobHandler(req, res) {
  ...
  try {
    const result = await runBillingTrialLifecycleReconcilerJob(supabase, {
      source: `http_job:${auth.mode}`,
      batchLimit: 100,
    });
    return ok(res, { ...result, auth_mode: auth.mode }, { traceId });
  } catch (err) {
    return fail(res, 500, "trial_lifecycle_reconciler_failed", {
      traceId,
